...
async function loadTasks() {
    ...
    for (const t of tasks) {
        ...
        try {
            ...
            tagsHTML = tags.map((tag) => `🏷️ ${tag.nome}`).join(", ");
        }
        catch { }
        // COMMENTS
        ...
        try {
            ...
            commentsHTML = comments.map((c) => `<div style="margin-left:10px;">💬 ${c.conteudo}</div>`).join("");
        }
        catch { }
        div.innerHTML = `
      <h3>${t.titulo} (#${t.id})</h3>
...
async function addTagUI(taskId) {
